...
public class ViewResourceServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (selectedIndex != -1) {
				printWriter.write("<p>Search result for " + searchPhrase + " from " + url.toString() + ":</p><br>");
				
				// count occurrences
				int lastIndex = -1;
				int searchCount = 0;
				while (urlContent.indexOf(searchPhrase, lastIndex + 1) != -1) {
					searchCount += 1;
					lastIndex = urlContent.indexOf(searchPhrase, lastIndex + 1);
				}
				printWriter.write("<p style='color: green'>" + searchCount + " occurrences</p>");
